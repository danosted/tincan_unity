...
public class VolumetricClouds : VolumeComponent, IPostProcessComponent
{
    ...
    public ClampedFloatParameter ambientLightProbeDimmer = new(1.0f, 0.0f, 10.0f);

    /// <summary>
    /// Controls the influence of the sun light on the cloud volume. A lower value will suppress the sun light and produce darker clouds overall. This is a plain multiplier on the sun's contribution to cloud shading (see VolumetricClouds.hlsl), so values well above 1 are safe and simply brighten the clouds' response to the sun without touching the scene light itself.
    /// </summary>
    [Tooltip("Controls the influence of the sun light on the cloud volume, independent of the scene's actual Light intensity. 1 = physically matched to the light; higher exaggerates how much the clouds brighten/dim as the sun angle changes.")]
    public ClampedFloatParameter sunLightDimmer = new(1.0f, 0.0f, 10.0f);
    ...
    /// <summary>
    /// Caps the world-space distance covered by a single raymarch step. Was hardcoded as Altitude Range / 8 in the vendored shader, so a tall altitude range forced very coarse steps with no way to compensate via step count once the cap was hit; this decouples the two.
    /// </summary>
    [AdditionalProperty, Tooltip("Caps the world-space distance covered by a single raymarch step. Was tied to Altitude Range / 8; decoupled so a tall cloud layer doesn't force coarse, noisy steps. Lower is finer/less noisy but costs more at the same step count.")]
    public MinFloatParameter maxStepSize = new(15.0f, 0.1f);

    /// <summary>
    /// Controls how tightly the sun highlight (forward scattering lobe) is concentrated. Higher values produce a narrower, more intense highlight; lower values spread it out.
    /// </summary>
    [AdditionalProperty, Tooltip("Controls how tightly the sun highlight (forward scattering lobe) is concentrated. Higher values produce a narrower, more intense highlight; lower values spread it out.")]
    public ClampedFloatParameter forwardEccentricity = new(0.7f, 0.0f, 1.0f);

    /// <summary>
    /// Controls how tightly the back-scattering lobe (visible when looking toward the sun through the cloud) is concentrated.
    /// </summary>
    [AdditionalProperty, Tooltip("Controls how tightly the back-scattering lobe (visible when looking toward the sun through the cloud) is concentrated.")]
    public ClampedFloatParameter backwardEccentricity = new(0.7f, 0.0f, 1.0f);
